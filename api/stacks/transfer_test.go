package stacks

import (
	"encoding/json"
	"gin-web/api/v1/wallet"
	"gin-web/models"
	"github.com/stretchr/testify/require"
	"math/big"
	"testing"
)

func TestTransfer(t *testing.T) {
	result, err := wallet.Transfer("598d99970d04be67e8b41ddd5c5453487eeab5345ea1638c9a2849dee377f2a3", "SP2P58SJY1XH6GX4W3YGEPZ2058DD3JHBPJ8W843Q", "20", big.NewInt(3000), big.NewInt(8), big.NewInt(200))
	require.NoError(t, err)
	var s wallet.TransactionRes
	err = json.Unmarshal([]byte(result), &s)
	require.NoError(t, err)
	expected := ""
	require.Equal(t, expected, s.TxSerialize)
}

func TestTransfer_index_compressedAddress(t *testing.T) {
	result, err := wallet.Transfer("598d99970d04be67e8b41ddd5c5453487eeab5345ea1638c9a2849dee377f2a301", "SP2P58SJY1XH6GX4W3YGEPZ2058DD3JHBPJ8W843Q", "20", big.NewInt(3000), big.NewInt(8), big.NewInt(200))
	require.NoError(t, err)
	var s wallet.TransactionRes
	err = json.Unmarshal([]byte(result), &s)
	require.NoError(t, err)
	expected := ""
	require.Equal(t, expected, s.TxSerialize)
}

// 新交易
func SignSwapTransaction(privateKeyHex string) ([]byte, error) {
	transferSig := models.StacksTransferSig{
		TokenX:       "SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2",
		TokenY:       "SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt",
		AmountIn:     big.NewInt(100000000000), // dx
		MinAmountOut: big.NewInt(166043736494), // min-dy
		Fees:         big.NewInt(180000000),    // 1.80 STX
		Nonce:        884,
	}

	signature, err := SignStacksTransfer(privateKeyHex, transferSig)
	if err != nil {
		return nil, err
	}
	return signature, nil
}
