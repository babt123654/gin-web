...
// 改
func Serialize1(stacksTransaction StacksTransaction) []byte {
	buf := bytes.NewBuffer(make([]byte, 0))
	buf.Write(getBytes(int64(stacksTransaction.Version), 0))
	chainIdBuffer := bytes.NewBuffer(make([]byte, 0, 4))
	...
	buffer2 := serializeLPList1(stacksTransaction.PostConditions)
	buf.Write(buffer2)

	buffer3 := SerializePayload1(stacksTransaction.Payload)
	buf.Write(buffer3)
...
// 改
func SerializePayload1(payload Payload) []byte {
	bufferArray := bytes.NewBuffer([]byte{})
	bufferArray.Write(getBytes(int64(payload.getPayloadType()), 0))
	switch payload.getPayloadType() {
	case 0:
...
func serializeStacksMessage1(message Message) []byte {
	switch message.GetType() {
	case MEMOSTRING:
		return serializeMemoString(message.(Memo))
	case ADDRESS:
...
func serializeLPList1(lpList LPList) []byte {
	bufferArray := bytes.NewBuffer(make([]byte, 0, 4096))
	list := lpList.Values
	bufferArray.Write(fromHexString(intToHexString(len(list), &lpList.LengthPrefixBytes)))
	for i := 0; i < len(list); i++ {
		fmt.Println(list[i].GetType())
		bufferArray.Write(serializeStacksMessage1(list[i]))
	}
	return sliceByteBuffer(bufferArray)
}
...
// 改
func serializeCV1(value Message) []byte {
	switch value.GetType() {
	case 0:
		return serializeIntCV(value.(*IntCV))
	case 1:
	...
	case 10:
		// 处理指针和非指针情况
		if someCV, ok := value.(*SomeCV); ok {
			return serializeOptionalCV(someCV)
		} else if someCV, ok := value.(SomeCV); ok {
			return serializeOptionalCV(&someCV)
		}
	case 9:
		return serializeNoneCV()
	case 6:
		if cp, ok := value.(*ContractPrincipalCV); ok {
			return serializeContractPrincipalCV(cp)
		} else if cp, ok := value.(ContractPrincipalCV); ok {
			return serializeContractPrincipalCV(&cp)
		}
	case 7, 8:
		return serializeResponseCV(value.(*ResponseCV))
