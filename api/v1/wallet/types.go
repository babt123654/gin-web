...
// 改
func NewContractPrincipalCV1(principal string) (ContractPrincipalCV, error) {
	if !strings.Contains(principal, ".") {
		return ContractPrincipalCV{}, fmt.Errorf("not contract address")
	}
	splitPrincipal := strings.Split(principal, ".")
	address := splitPrincipal[0]
	contractName := splitPrincipal[1]
	...
	lengthPrefixedContractName := createLPString(contractName, nil, nil)
	if len(lengthPrefixedContractName.Content) >= 128 {
		return ContractPrincipalCV{}, fmt.Errorf("contract name must be less than 128 bytes")
	}
	return ContractPrincipalCV{
		PrincipalContract,
		*addr,
